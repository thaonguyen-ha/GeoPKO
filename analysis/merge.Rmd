...
A note on this: to use the published Geo-PKO dataset, you'll need to merge ViEWS' `month_id` with Geo-PKO's `month' and `year` fields. This page will be updated when the new dataset is published, including how to do this merge.

### Filtering, joining and subsetting the datasets at the PRIO-grid level

The Geo-PKO dataset we're working with includes data from 1994 onwards. The predictor database is current as at July 2020 and forecasts the next 36 months ahead. So we'll filter Geo-PKO data to include the year prior to July 2020. Both the Geo-PKO and ViEWS datasets include `pg_id`, a unique code that corresponds to a specific grid square on the map. This is what we'll use to merge the datasets.

```{r}
# filtering for troop deployments over the most recent year
geopkoviews2 <- geopkoviews %>%
  filter(between(month_id, 510, 522))
...
To work with the data within this shapefile, which includes geospatial polygons and data that corresponds to the ViEWS dataset, we need to fortify the shapefile. We also convert the IDs to rownames to make it easier to work with. And, finally, we merge it with `pgnewdf2`, which we created earlier.
