...
```{r}
library(ggrepel)
mybreaks <- c(0, 100, 300, 500, 1000, 2000, 3000, 4000,5000)
p2 <-  ggplot(data=Africa) + geom_sf() + 
  geom_point(data = map2018df1, aes(x=longitude, y=latitude, size= ave, color= ave), alpha=.7)+
  scale_size_continuous(name="Average Troop Deployment", trans="log", range=c(1,12), breaks=mybreaks) +
  scale_alpha_continuous(name="Average Troop Deployment", trans="log", range=c(0.1, .9), breaks=mybreaks)+
  scale_color_viridis(option="cividis", trans="log", breaks=mybreaks, name="Average Troop Deployment" ) +
  guides( colour = guide_legend()) +
  geom_point(data = map2018df1 %>% filter(HQ==3), aes (x=longitude, y=latitude), color = "red", shape = 3, size=10)+
