...
It can so be derived that 21 missions are active in only 1 country. Only 1 mission is active in 9 countries at the same time.
```{r}
...
### UNMO and UNPOL
The following shows how to extract locations that have both UNPOL and UNMO deployments. 
A total of 1286 have a combination of UNPOL and UNMO deployment.
```{r}
MOPOL <- GeoPKO %>% select(Mission, year, month, location, UNPOL..dummy., UNMO..dummy.) %>%filter(UNMO..dummy.>0, UNPOL..dummy.>0)
kable(MOPOL[1:20,]) %>% kable_styling() %>%
  scroll_box(width = "100%", height = "200px") #displaying the first five rows
nrow(MOPOL)
```
