...
This snippet shows that R might arbitrarily parse our columns as a random mix of logical or character variables. Here, we are going to dodge this issue by telling R to parse all columns as character. Running `str()` can help us ensure that the process was carried out correctly. The next two code lines show us that the dataframe contains `r nrow(GeoPKO)` observations and `r ncol(GeoPKO)` columns. 
...
ncol(GeoPKO)
```
The output from `str()` could get rather clunky, so here is a prettier snippet produced by `kableExtra`.
